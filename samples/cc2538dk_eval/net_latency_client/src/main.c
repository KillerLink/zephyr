...
LOG_MODULE_REGISTER(net_latency_client, LOG_LEVEL_DBG);
...
void send_data() {
	int ret = 0;
	LOG_DBG("send(%d, %p, %d, 0);\n", sock, lorem_ipsum, ret);
	EVAL_LED1_TOGGLE;
	ret = send(sock, lorem_ipsum, DATA_LENGTH, 0);
	EVAL_LED2_TOGGLE;
	if (ret < 0) {
		LOG_DBG("send(...) failed: %d", ret);
	} else {
		LOG_DBG("send(...) sent %d bytes", ret);
	}
}
...
void main(void) {
	...
	while (1) {
		send_data();
		k_sleep(K_SECONDS(MESSAGE_DELAY));
	}
}
